...
function Primitive({ item }: { item: SceneObject }) {
  ...
  return (
    <mesh position={position} rotation={rotation} scale={scale} castShadow receiveShadow>
      ...
      <meshStandardMaterial color={item.color} roughness={item.material === "glass" ? 0.12 : 0.72} metalness={item.material === "metal" ? 0.65 : 0.05} transparent={item.material === "glass"} opacity={item.material === "glass" ? 0.58 : 1} />
    </mesh>
  );
}

export function ScenePreview({ scene }: { scene: SceneSpec | null }) {
  return (
    <div className="preview-shell" aria-label="Prévia 3D do mapa">
      <Canvas shadows dpr={[1, 1.7]}>
        ...
        <OrbitControls makeDefault minDistance={8} maxDistance={150} maxPolarAngle={Math.PI / 2.03} />
      </Canvas>
      {!scene && (
